...
class FolderViewController: UITableViewController {
	...
	override func viewDidLoad() {
		...
		viewModel.folderContents.asObservable().bind(to: tableView.rx.items(dataSource: dataSource)).disposed(by: disposeBag)
	...
	@IBAction func createNewRecording(_ sender: Any) {
		delegate?.createRecording(in: viewModel.folder)
	}
	...
	override func encodeRestorableState(with coder: NSCoder) {
		...
		coder.encode(viewModel.folder.uuidPath, forKey: .uuidPathKey)
	}
