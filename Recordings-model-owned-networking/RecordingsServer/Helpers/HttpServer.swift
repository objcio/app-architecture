...
fileprivate class SocketListener: NSObject, NetServiceDelegate {
	...
	// In the current implementation, the server always runs on a hard-coded port and on all interfaces.
	let serverPort: UInt16 = 47328
	...
	init(serverInfo: HttpServerInfo, handlers: Array<HttpResponseHandler.Type>) throws {
		...
		var addr_in6 = sockaddr_in6(sin6_len: UInt8(MemoryLayout<sockaddr_in6>.size), sin6_family: UInt8(AF_INET6), sin6_port: UInt16(serverPort).bigEndian, sin6_flowinfo: 0, sin6_addr: serverIpv6Address, sin6_scope_id: 0)
		...
		netService = NetService(domain: "local.", type: "_\(serverInfo.serviceName)._tcp.", name: "", port: Int32(serverPort))
