...
class PlayViewController: UIViewController, UITextFieldDelegate, AVAudioPlayerDelegate {
	...
	func updateForChangedRecording() {
		if let r = recording, let url = r.streamURL {
	...
	override func decodeRestorableState(with coder: NSCoder) {
		...
		if let uuidPath = (coder.decodeObject(forKey: .uuidPathKey) as? [String])?.compactMap({ UUID(uuidString: $0) }), let recording = Store.shared.item(atUUIDPath: uuidPath) as? Recording {
